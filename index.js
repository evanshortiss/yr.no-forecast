...
LocationForecast.prototype = {
    _init: function(callback) {
        ...
        async.forEach(json['weatherdata']['product']['time'], function(
            time, cb) {
        ...
        for (var i = 0; i < 5; i++) {
            ...
            (function() {
                ...
                fns.push(function(cb) {
                    self.getForecastForTime(day, function(
                        err, weather) {
                        if (err) {
                            return cb(err, null);
                        }
    ...
    getForecastForTime: function(time, callback) {
        ...
        if (time.isValid() === false) {
            return callback(
                'Invalid date provided for weather lookup. Date: ' +
                time);
        }
        ...
        getBasicForTime(time, self.basic, function(err, basic) {
            ...
            getDetailForTime(time, self.detail, function(err,
                detail) {
                if (detail) {
                    return callback(null, buildDetail(
                        detail, basic));
                }
                return callback(null, res);
            });
...
function buildDetail(detail, basic) {
    ...
    var obj = {
        iconId: basic['location']['symbol']['number'],
        icon: basic['location']['symbol']['id'],
        to: basic['to'],
        from: basic['from'],
        rain: (basic['location']['precipitation']['value'] + ' ' + basic[
...
function getBasicForTime(date, basic, callback) {
    ...
    getItemsForDay(basic, date, function(err, items) {
        ...
        async.each(items, function(item, cb) {
            ...
            if ((from.isSame(date) || from.isBefore(date)) && (
                    to.isSame(date) || to.isAfter(date))) {
...
function getItemsForDay(collection, date, callback) {
    ...
    var res = [];
    async.each(collection, function(item, cb) {
        if (moment.utc(item.from).isSame(date, 'day') || moment.utc(
                item.to).isSame(date, 'day')) {
            res.push(item);
        }
        cb();
    }, function(err) {
